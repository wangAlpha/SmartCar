 ...
 *    无
 */
uint8_t LPLD_MAG3110_Init(void)
{
    uint8_t device_id;
 ...
 *    |__ 具体写入的字节型数据
 */
void LPLD_MAG3110_WriteReg(uint8_t RegisterAddress, uint8_t Data)
{
    //发送从机地址
 ...
 *      加速传感器的测量值
 */
uint8_t LPLD_MAG3110_ReadReg(uint8_t RegisterAddress)
{
    uint8_t result;
    //发送从机地址
    LPLD_I2C_StartTrans(MAG3110_I2CX, MAG3110_DEV_ADDR, I2C_MWSR);
 ...
 * 函数返回值：加速度值（int8_t）
 */
int16_t LPLD_MAG3110_GetResult(uint8_t Status, uint8_t Regs_Addr)
{
    uint8_t ret = 0;
    uint16_t cnt = 0;
    int16_t result, temp;
