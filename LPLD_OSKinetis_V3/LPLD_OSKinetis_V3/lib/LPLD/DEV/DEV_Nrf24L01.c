...
static uint8_t LPLD_Nrf24L01_WriteBuf(uint8_t reg, uint8_t *pbuf , uint8_t len);
static uint8_t LPLD_Nrf24L01_ReadBuf(uint8_t reg, uint8_t *pbuf, uint8_t len);
static uint8_t LPLD_Nrf24L01_WriteReg(uint8_t reg, uint8_t value);
static uint8_t LPLD_Nrf24L01_ReadReg(uint8_t reg);
static uint8_t LPLD_Nrf24L01_CheckID(void);
static void Nrf24L01_DelayUs(uint16_t t);

uint8_t TX_ADDRESS[NRF24L01_TX_ADR_LEN] = {0x34, 0x43, 0x10, 0x10, 0x02}; //发送地址
uint8_t RX_ADDRESS[NRF24L01_RX_ADR_LEN] = {0x34, 0x43, 0x10, 0x10, 0x02}; //接收地址(本机地址,仅用于接收模式)
//NRF24L01复位之后用于检测
// NRF24L01_RX_ADDR_P0的复位值是否为0xE7E7E7E7
// NRF24L01_RX_ADDR_P1的复位值是否为0xC2C2C2C2
// NRF24L01_TX_ADDR的复位值是否为0xE7E7E7E7
uint8_t NRF24L01_RX_ADDR_P0_RESET_BUF[5];
uint8_t NRF24L01_RX_ADDR_P1_RESET_BUF[5];
uint8_t NRF24L01_TX_ADDR_RESET_BUF[5];
 ...
 *    1 配置成功
 */
uint8_t LPLD_Nrf24L01_Init(void)
{
    //配置SPI
 ...
 *    Nrf24L01寄存器返回状态
 */
uint8_t LPLD_Nrf24L01_TxModeInit(void)
{
    uint8_t Temp;
    NRF24L01_CE_L;
    // 芯片上电复位后
 ...
 *    Nrf24L01寄存器返回状态
 */
uint8_t LPLD_Nrf24L01_RxModeInit(void)
{
    uint8_t Temp;
    NRF24L01_CE_L;
    // 芯片上电复位后
 ...
 *    0 发送失败
 */
uint8_t LPLD_Nrf24L01_SendFrame(uint8_t *data, uint8_t len)
{
    uint8_t Status;
 ...
 *    无
 */
void LPLD_Nrf24L01_RecvFrame(uint8_t *data, uint8_t len)
{
    uint8_t Status;
