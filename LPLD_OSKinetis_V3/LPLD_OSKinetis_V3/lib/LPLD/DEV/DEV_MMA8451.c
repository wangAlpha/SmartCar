 ...
 *      设备ID
 */
uint8_t LPLD_MMA8451_Init(void)
{
 ...
 *    |__ 具体写入的字节型数据
 */
void LPLD_MMA8451_WriteReg(uint8_t RegisterAddress, uint8_t Data)
{
    //发送从机地址
 ...
 *      加速传感器的测量值
 */
uint8_t LPLD_MMA8451_ReadReg(uint8_t RegisterAddress)
{
    uint8_t result;
    //发送从机地址
    LPLD_I2C_StartTrans(MMA8451_I2CX, MMA8451_DEV_ADDR, I2C_MWSR);
 ...
 * 函数返回值：加速度值（int16_t）
 */
int16_t LPLD_MMA8451_GetResult(uint8_t Status, uint8_t Regs_Addr)
{
    uint8_t ret = 0;
    uint16_t cnt = 0;
    int16_t result, temp;
