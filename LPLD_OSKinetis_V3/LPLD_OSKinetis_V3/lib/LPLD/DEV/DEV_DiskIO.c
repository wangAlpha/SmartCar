...
DSTATUS LPLD_Disk_Initialize(uint8_t drv)
{
    if (drv) {
        return RES_PARERR;
    }
 ...
 *    RES_NONRSPNS--未响应
 */
DSTATUS LPLD_Disk_Status(uint8_t drv)
{
 ...
 *    RES_NONRSPNS--未响应
 */
DRESULT LPLD_Disk_Read(uint8_t drv, uint8_t *buff, uint32_t sector, uint8_t count)
{
    if (drv || (count == 0)) {
        return RES_PARERR;
    }
...
DRESULT LPLD_Disk_Write(uint8_t drv, const uint8_t *buff, uint32_t sector, uint8_t count)
{
    if (drv || (count == 0)) {
        return RES_PARERR;
    }
    return LPLD_SDHC_WriteBlocks((uint8_t *)buff, sector, count);
}
#endif
 ...
 *    RES_NONRSPNS--未响应
 */
DRESULT LPLD_Disk_IOC(uint8_t drv, uint8_t ctrl, void *buff)
{
    ...
    case GET_SECTOR_SIZE:
        ...
        */
        if (buff == NULL) {
            ...
            *(uint32_t *)buff = IO_SDCARD_BLOCK_SIZE;
        }
        break;
    case GET_SECTOR_COUNT:
        ...
        */
        if (buff == NULL) {
            ...
            *(uint32_t *)buff = sdcard_ptr->NUM_BLOCKS;
        }
        break;
    case GET_BLOCK_SIZE:
        /*
