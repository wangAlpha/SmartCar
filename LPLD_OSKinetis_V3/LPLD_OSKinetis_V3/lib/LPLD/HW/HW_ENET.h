...
/* 描述符数组空间 */
static uint8_t xENETTxDescriptors_unaligned[(CFG_NUM_ENET_TX_BUFFERS * sizeof(ENET_NbufTypeDef)) + 16 ];
static uint8_t xENETRxDescriptors_unaligned[(CFG_NUM_ENET_RX_BUFFERS * sizeof(ENET_NbufTypeDef)) + 16 ];
static uint8_t ucENETRxBuffers[(CFG_NUM_ENET_RX_BUFFERS * CFG_ENET_RX_BUFFER_SIZE) + 16 ];
...
static uint32_t uxNextRxBuffer = 0, uxNextTxBuffer = 0;

//ENET模块初始化
void LPLD_ENET_Init(ENET_InitTypeDef);
//以太帧发送函数
void LPLD_ENET_MacSend(uint8_t *, uint16_t);
//以太帧接收函数
uint8_t LPLD_ENET_MacRecv(uint8_t *, uint16_t *);
//生成给定的MAC地址的哈希表
uint8_t LPLD_ENET_HashAddress(const uint8_t *);
//设置MAC物理地址
void LPLD_ENET_SetAddress(const uint8_t *);
//设置ENET模块的MII接口时钟
void LPLD_ENET_MiiInit(uint32_t);
//MII接口写
uint8_t LPLD_ENET_MiiWrite(uint16_t , uint16_t, uint16_t);
//MII接口读
uint8_t LPLD_ENET_MiiRead(uint16_t, uint16_t, uint16_t *);
