 ...
 *    1--配置成功
 */
uint8_t LPLD_LPTMR_Init(LPTMR_InitTypeDef lptmr_init_structure)
{
    uint8_t  mode = lptmr_init_structure.LPTMR_Mode;
    uint16_t period_ms = lptmr_init_structure.LPTMR_PeriodMs;
    uint8_t  pluseacc_input = lptmr_init_structure.LPTMR_PluseAccInput;
    ...
    if (mode == LPTMR_MODE_TIMER) {       // 配置成低功耗定时器功能
        ...
        LPTMR0->CMR = LPTMR_CMR_COMPARE(65535);                 //设置比较值
 ...
 *    1--配置成功
 */
uint8_t LPLD_LPTMR_Deinit(void)
{
    //清空CSR寄存器
    LPTMR0->CSR = 0x00;
    //清空PSR寄存器
 ...
 *
 */
uint8_t LPLD_LPTMR_EnableIrq(void)
{
 ...
 *
 */
uint8_t LPLD_LPTMR_DisableIrq(void)
{
    disable_irq(LPTimer_IRQn);
    return 1;
}
