...
static void LPLD_ENET_Delay(uint32_t time);
...
void LPLD_ENET_Init(ENET_InitTypeDef enet_init_struct)
{
    ...
    if (usData & PHY_DUPLEX_STATUS) {
        //全双工
        ENET->RCR &= (uint32_t)~ENET_RCR_DRT_MASK;
    ...
    //指向环形接收缓冲区描述符序列的头地址
    ENET->RDSR = (uint32_t) & (xENETRxDescriptors[ 0 ]);
    //指向环形发送缓冲区描述符序列的头地址
    ENET->TDSR = (uint32_t) xENETTxDescriptors;
    //清除ENET中断事件
    ENET->EIR = (uint32_t) - 1;
    //使能中断
    ENET->EIMR = 0
 ...
 *
 */
uint8_t LPLD_ENET_SetIsr(uint8_t type, ENET_ISR_CALLBACK isr_func)
{
    if (type > 4) {
        return 0;
    }
 ...
 *    无
 */
void LPLD_ENET_MacSend(uint8_t *ch, uint16_t len)
{
    //检查当前发送缓冲区描述符是否可用
    while (xENETTxDescriptors[ uxNextTxBuffer ].status & TX_BD_R);
    //设置发送缓冲区描述符
    xENETTxDescriptors[ uxNextTxBuffer ].data = (uint8_t *)__REV((uint32_t)ch);
 ...
 *    无
 */
uint8_t LPLD_ENET_MacRecv(uint8_t *ch, uint16_t *len)
{
    uint8_t *prvRxd;
    *len = 0;
    uxNextRxBuffer = 0;
    ...
    *len  =  __REVSH(xENETRxDescriptors[ uxNextRxBuffer ].length);
    prvRxd = (uint8_t *)__REV((uint32_t)xENETRxDescriptors[ uxNextRxBuffer ].data);
 ...
 *    32位CRC校验的高6位
 */
uint8_t LPLD_ENET_HashAddress(const uint8_t *addr)
{
    uint32_t crc;
    uint8_t byte;
    int i, j;
 ...
 *    无
 */
void LPLD_ENET_SetAddress(const uint8_t *pa)
{
    uint8_t crc;
    //设置物理地址
 ...
 *    无
 */
void LPLD_ENET_MiiInit(uint32_t sys_clk_mhz)
{
 ...
 *    0--写入成功
 */
uint8_t LPLD_ENET_MiiWrite(uint16_t phy_addr, uint16_t reg_addr, uint16_t data)
{
    uint32_t timeout;
    //清除MII中断事件
 ...
 *    0--读取成功
 */
uint8_t LPLD_ENET_MiiRead(uint16_t phy_addr, uint16_t reg_addr, uint16_t *data)
{
    uint32_t timeout;
    //清除MII中断事件
    ENET->EIR = ENET_EIR_MII_MASK;
