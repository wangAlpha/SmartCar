...
//通道基准值
uint16_t TSI_ChxBaseVal[16];

const uint16_t *TSI_CNTR[16] = {(uint16_t *) &TSI0->CNTR1, (uint16_t *)(&TSI0->CNTR1) + 1,
                                (uint16_t *) &TSI0->CNTR3, (uint16_t *)(&TSI0->CNTR3) + 1,
                                (uint16_t *) &TSI0->CNTR5, (uint16_t *)(&TSI0->CNTR5) + 1,
                                (uint16_t *) &TSI0->CNTR7, (uint16_t *)(&TSI0->CNTR7) + 1,
                                (uint16_t *) &TSI0->CNTR9, (uint16_t *)(&TSI0->CNTR9) + 1,
                                (uint16_t *) &TSI0->CNTR11, (uint16_t *)(&TSI0->CNTR11) + 1,
                                (uint16_t *) &TSI0->CNTR13, (uint16_t *)(&TSI0->CNTR13) + 1,
                                (uint16_t *) &TSI0->CNTR15, (uint16_t *)(&TSI0->CNTR15) + 1
                               };

const uint32_t *TSI_PORT[32] = {(uint32_t *) &PORTB->PCR[0], (uint32_t *) &PORTA->PCR[0], (uint32_t *) &PORTA->PCR[1], (uint32_t *) &PORTA->PCR[2],
                                (uint32_t *) &PORTA->PCR[3], (uint32_t *) &PORTA->PCR[4], (uint32_t *) &PORTB->PCR[1], (uint32_t *) &PORTB->PCR[2],
                                (uint32_t *) &PORTB->PCR[3], (uint32_t *) &PORTB->PCR[16], (uint32_t *) &PORTB->PCR[17], (uint32_t *) &PORTB->PCR[18],
                                (uint32_t *) &PORTB->PCR[19], (uint32_t *) &PORTC->PCR[0], (uint32_t *) &PORTC->PCR[1], (uint32_t *) &PORTC->PCR[2]
                               };
 ...
 *    1--配置成功
 */
uint8_t LPLD_TSI_Init(TSI_InitTypeDef tsi_init_struct)
{
    uint16_t chs = tsi_init_struct.TSI_Chs;
    uint8_t mode = tsi_init_struct.TSI_ScanTriggerMode;
    uint8_t esor = tsi_init_struct.TSI_EndScanOrOutRangeInt;
    ...
    for (uint8_t i = 0; i < 16; i++) {
        if ((chs >> i) & 0x1L) {
            *((uint32_t *)TSI_PORT) = PORT_PCR_MUX(0);
        }
    }
    //配置通用控制寄存器
    ...
    if (tsi_init_struct.TSI_IsInitSelfCal == TRUE) {
        ...
        for (uint8_t i = 0; i < 16; i++) {
            if ((chs >> i) & 0x1L) {
                TSI_ChxBaseVal[i] = *((uint16_t *)TSI_CNTR[i]);
#if defined(CPU_MK60DZ10)
 ...
 *    FALSE-未触摸
 */
boolean LPLD_TSI_IsChxTouched(uint8_t ch_num)
{
    if (TSI_ChxBaseVal[ch_num] < * ((uint16_t *)TSI_CNTR[ch_num])) {
        return TRUE;
    }
    return FALSE;
}
