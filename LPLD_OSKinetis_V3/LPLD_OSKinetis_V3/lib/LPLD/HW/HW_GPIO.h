...
//GPIO通用初始化函数
uint8_t LPLD_GPIO_Init(GPIO_InitTypeDef);
//使能GPIO外部中断
uint8_t LPLD_GPIO_EnableIrq(GPIO_InitTypeDef);
//禁用GPIO外部中断
uint8_t LPLD_GPIO_DisableIrq(GPIO_InitTypeDef);
//设置GPIO端口0~31位的输出
void LPLD_GPIO_Output(GPIO_Type *, uint32_t);
//设置GPIO端口一位的输出
void LPLD_GPIO_Output_b(GPIO_Type *, uint32_t, uint8_t);
//设置GPIO端口8位的输出
void LPLD_GPIO_Output_8b(GPIO_Type *, uint8_t, uint8_t);
//设置GPIO端口0~31的电平翻转
void LPLD_GPIO_Toggle(GPIO_Type *, uint32_t);
//设置GPIO端口一位的翻转
void LPLD_GPIO_Toggle_b(GPIO_Type *, uint8_t);
//设置GPIO端口8位的翻转
void LPLD_GPIO_Toggle_8b(GPIO_Type *, uint8_t, uint8_t);
//取得GPIO口0~31位的数据
uint32_t LPLD_GPIO_Input(GPIO_Type *);
//取得GPIO口某一位的数据
uint8_t LPLD_GPIO_Input_b(GPIO_Type *, uint8_t);
//取得GPIO端口8位数据的输入
uint8_t LPLD_GPIO_Input_8b(GPIO_Type *, uint8_t);
