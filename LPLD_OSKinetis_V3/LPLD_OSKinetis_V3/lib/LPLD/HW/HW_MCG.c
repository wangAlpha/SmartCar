...
uint32_t g_core_clock = -1ul;
uint32_t g_bus_clock = -1ul;
uint32_t g_flash_clock = -1ul;
uint32_t g_flexbus_clock = -1ul;
 ...
 *    内核频率，单位MHz
 */
uint8_t LPLD_PLL_Setup(PllOptionEnum_Type core_clk_mhz)
{
    uint8_t pll_freq;
    uint8_t prdiv, vdiv;
    uint8_t core_div, bus_div, flexbus_div, flash_div;
    ...
    */
    ...
    core_div = 0;
    if ((bus_div = (uint8_t)(core_clk_mhz / BUS_CLK_MHZ - 1u)) == (uint8_t) - 1) {
        bus_div = 0;
    }
    else if (core_clk_mhz / (bus_div + 1) > BUS_CLK_MHZ) {
        bus_div += 1;
    }
    if ((flexbus_div = (core_clk_mhz / FLEXBUS_CLK_MHZ - 1u)) == (uint8_t) - 1) {
        flexbus_div = 0;
    }
    else if (core_clk_mhz / (flexbus_div + 1) > FLEXBUS_CLK_MHZ) {
        flexbus_div += 1;
    }
    if ((flash_div = (core_clk_mhz / FLASH_CLK_MHZ - 1u)) == (uint8_t) - 1) {
        flash_div = 0;
    }
 ...
 */
RAMFUNC void LPLD_Set_SYS_DIV(uint32_t outdiv1, uint32_t outdiv2, uint32_t outdiv3, uint32_t outdiv4)
{
    uint32_t temp_reg;
    uint8_t i;
