...
#include "HW_CAN.h"
//FlexCAN模块设置波特率函数
static void LPLD_CAN_SetBaudRate(CAN_Type *, uint32_t);
//FlexCAN 清楚中断标志位函数
static void LPLD_CAN_Interrupt_ClearPending(CAN_Type *, uint8_t);
//FlexCAN 获得中断标志位函数
static uint8_t LPLD_CAN_Interrupt_GetFlag(CAN_Type *, uint8_t);
//FlexCAN 解锁Msg缓冲区函数
static uint16_t LPLD_CAN_UnlockMsg(CAN_Type *);
//FlexCAN 使能Msg缓冲区中断函数
static void LPLD_CAN_EnableMsgInterrupt(CAN_Type *, uint8_t);
//FlexCAN 设置Msg缓冲区的CODE
static void LPLD_CAN_SetMsgCode(CAN_Type *, uint8_t , uint8_t);
//FlexCAN 获得Msg缓冲区的CODE
static uint32_t LPLD_CAN_GetMsgCode(CAN_Type *, uint8_t);
//FlexCAN 获得Msg缓冲区的TIMESTAMP
static uint16_t LPLD_CAN_GetMsgTimeStamp(CAN_Type *, uint8_t);
//FlexCAN 设置Msg缓冲区的ID
void LPLD_CAN_SetMsgID(CAN_Type *, uint8_t , uint8_t, uint32_t);
//FlexCAN 获得Msg缓冲区的ID
static uint32_t LPLD_CAN_GetMsgID(CAN_Type *, uint8_t);
//FlexCAN 获得Msg缓冲区的长度
uint8_t LPLD_CAN_GetMsgLength(CAN_Type *canx, uint8_t MSG_NUM);
//FlexCAN 获得Msg缓冲区的数据
static void LPLD_CAN_GetData(CAN_Type *canx, uint8_t , uint8_t ,  uint8_t *);
//FlexCAN 写Msg缓冲区的数据
static void LPLD_CAN_WriteData(CAN_Type *, uint8_t , uint8_t, uint8_t *);
//FlexCAN 传输中断函数
void LPLD_CAN_Transmit_Interrupt(CAN_Type *, uint8_t);
//FlexCAN 接收中断函数
static void LPLD_CAN_Receive_Interrupt(CAN_Type *, uint8_t);
//FlexCAN CANx的中断处理函数
...
void LPLD_CAN_Init(CAN_InitTypeDef can_init_structure)
{
    ...
    uint32_t baud = can_init_structure.CAN_BaudRate;
    uint8_t  mask_mode = can_init_structure.CAN_RxMaskMode;
 ...
 *    无
*/
static void LPLD_CAN_SetBaudRate(CAN_Type *canx, uint32_t baud)
{
    uint8_t prescale;
 ...
 *
 */
void LPLD_CAN_InitMessageObject(CAN_MSGOBJ_InitTypeDef can_msg_init_structure, uint32_t rx_id)
{
    uint8_t     msg_num_temp  = can_msg_init_structure.CAN_MsgNum;
    uint8_t     dir           = can_msg_init_structure.CAN_MsgDirection;
    CAN_Type *canx_ptr      = can_msg_init_structure.CAN_Canx;
    uint32_t    id_type       = can_msg_init_structure.CAN_MsgIdLength;
    uint32_t    cs_temp = CAN_MB_CS_CODE(CAN_MSGOBJ_RX_EMPTY);
 ...
 *
 */
uint8_t LPLD_CAN_TransmitMessage(CAN_MSGOBJ_InitTypeDef can_msg_init_structure,
                                 CAN_MessageFormat_TypeDef *can_tx_msg)
{
    ...
    CAN_Type *canx_ptr   = can_msg_init_structure.CAN_Canx;
    uint32_t  id_type      = can_msg_init_structure.CAN_MsgIdLength;
    uint32_t  message_id = can_tx_msg->CAN_MsgID; ;
    uint8_t   priority = can_tx_msg->CAN_MsgPriority;
    uint8_t  *in_data_buffer = can_tx_msg->CAN_MsgDataBuffer;
    uint8_t   in_data_length = can_tx_msg->CAN_MsgDataLength;
    uint32_t  id_temp;
    uint32_t  cs_temp = CAN_MB_CS_RTR(can_msg_init_structure.CAN_MsgRTR) |
...
void LPLD_CAN_ReceivedMessage(CAN_MSGOBJ_InitTypeDef can_msg_init_structure, \
                              CAN_MessageFormat_TypeDef *can_rx_msg)
{
    uint32_t message_code, i;
    volatile uint16_t timer;
    uint8_t rx_data[CAN_DATA_MAX_BYTES];
    uint8_t data_length;
    uint16_t time_stamp;
    uint32_t msg_id;
    CAN_Type *canx = can_msg_init_structure.CAN_Canx;
    uint8_t msg_num = can_msg_init_structure.CAN_MsgNum;
    //获得Msg缓冲区中的code值
 ...
 *   无
 */
static void LPLD_CAN_Interrupt_ClearPending(CAN_Type *canx, uint8_t msg_num)
{
    if (msg_num < 32) {
 ...
 *    返回需要 Msg缓冲区的中断标志位
 */
static uint8_t LPLD_CAN_Interrupt_GetFlag(CAN_Type *canx, uint8_t msg_num)
{
    uint8_t status;
    if (msg_num < 32) {
 ...
 *    16位CAN自由计数器记录的时间戳
 */
static uint16_t LPLD_CAN_UnlockMsg(CAN_Type *canx)
{
    return (canx->TIMER);
}
 ...
 *
 */
static void LPLD_CAN_EnableMsgInterrupt(CAN_Type *canx, uint8_t msg_num)
{
    if (msg_num < 32) {
 ...
 *
 */
static void LPLD_CAN_SetMsgCode(CAN_Type *canx, uint8_t msg_num, uint8_t code)
{
 ...
 *
 */
static uint32_t LPLD_CAN_GetMsgCode(CAN_Type *canx, uint8_t msg_num)
{
 ...
 *
 */
static uint16_t LPLD_CAN_GetMsgTimeStamp(CAN_Type *canx, uint8_t msg_num)
{
 ...
 *
 */
static uint32_t LPLD_CAN_GetMsgID(CAN_Type *canx, uint8_t msg_num)
{
    uint32_t id_temp;
 ...
 *
 */
static uint8_t LPLD_CAN_GetMsgLength(CAN_Type *canx, uint8_t msg_num)
{
    ...
    uint8_t    *in_buffer)
{
    int i;
    uint8_t  *pMBData;
    uint8_t  *pRecieve  = in_buffer;
    uint8_t  word_num   = (in_length - 1) / 4; //Get the Message buffer word number
    uint8_t  rest_bytes = (in_length - 1) % 4; //Get the reset bytes of buffer
    if (word_num > 0) {
        pMBData = (uint8_t *)&canx->MB[msg_num].WORD0 + 3;
        for (i = 0 ; i < 4 ; i++) {
            pRecieve[i] = *pMBData--;
        }
        pMBData = (uint8_t *)&canx->MB[msg_num].WORD1 + 3;
    }
    else {
        pMBData = (uint8_t *)&canx->MB[msg_num].WORD0 + 3;
    }
    ...
    uint8_t    *in_buffer)
{
    int i;
    uint32_t word[2] = {0};
    uint8_t  word_num   = (in_length - 1) / 4; //获得 Msg 的Word 值
    uint8_t  rest_bytes = (in_length - 1) % 4; //获得剩余字节的值
    if (msg_num >= MSG_MAX_NO || in_length > 8) {
        return;
    }
    if (word_num < 1) {
        for (i = 0; i <= rest_bytes ; i++) {
            word[word_num] |= (uint32_t)in_buffer[i] << (24 - (i << 3));
        }
    }
    else {
        for (i = 0; i < 4; i++) {
            word[word_num - 1] |= (uint32_t)in_buffer[i] << (24 - (i << 3));
        }
        for (i = 0; i <= rest_bytes ; i++) {
            word[word_num] |= (uint32_t)in_buffer[i + 4] << (24 - (i << 3));
        }
    }
 ...
 *    无
 */
void LPLD_CAN_Transmit_Interrupt(CAN_Type *canx, uint8_t msg_num)
{
 ...
 *    无
 */
static void LPLD_CAN_Receive_Interrupt(CAN_Type *canx, uint8_t msg_num)
{
