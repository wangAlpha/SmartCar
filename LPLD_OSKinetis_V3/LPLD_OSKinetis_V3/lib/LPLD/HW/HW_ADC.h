...
uint8_t LPLD_ADC_Init(ADC_InitTypeDef);
//ADC反初始化函数，禁用ADC模块
uint8_t LPLD_ADC_Deinit(ADC_InitTypeDef);
//使能ADCx相应转换通道的AD采集功能
uint8_t LPLD_ADC_Chn_Enable(ADC_Type *, AdcChnEnum_Type);
//软件触发模式获取AD转换结果
uint16_t LPLD_ADC_Get(ADC_Type *, AdcChnEnum_Type);
//使能ADCx中断
uint8_t LPLD_ADC_EnableIrq(ADC_InitTypeDef);
//禁用ADCx中断
uint8_t LPLD_ADC_DisableIrq(ADC_InitTypeDef);
//使能ADCx转换通道，用于软件中断、硬件触发等模式
void LPLD_ADC_EnableConversion(ADC_Type *, AdcChnEnum_Type, uint8_t, boolean);
//获取AD转换结果
uint16_t LPLD_ADC_GetResult(ADC_Type *, uint8_t);
//获取SC1寄存器中COCO位置1的编号
uint8_t LPLD_ADC_GetSC1nCOCO(ADC_Type *);
