 ...
 *    1--配置成功
 */
uint8_t LPLD_SPI_Init(SPI_InitTypeDef spi_init_structure)
{
 ...
 *    1--配置成功
 */
uint8_t LPLD_SPI_Deinit(SPI_InitTypeDef spi_init_structure)
{
    SPI_Type *spix = spi_init_structure.SPI_SPIx;
    spix->MCR |= SPI_MCR_HALT_MASK; //
    if (spix == SPI0) {
 ...
 *    1--配置成功
 */
uint8_t LPLD_SPI_EnableIrq(SPI_InitTypeDef spi_init_structure)
{
    SPI_Type *spix = spi_init_structure.SPI_SPIx;
    if (spix == SPI0) {
 ...
 *    1--配置成功
 */
uint8_t LPLD_SPI_DisableIrq(SPI_InitTypeDef spi_init_structure)
{
    SPI_Type *spix = spi_init_structure.SPI_SPIx;
    if (spix == SPI0) {
        disable_irq(SPI0_IRQn);
 ...
 *    读取从机8位的数据
 */
uint8_t LPLD_SPI_Master_WriteRead(SPI_Type *spix, uint8_t data, uint8_t pcsx, uint8_t pcs_state)
{
    uint8_t temp;
 ...
 *    读取从机8位的数据
 */
uint8_t LPLD_SPI_Master_Read(SPI_Type *spix)
{
    uint8_t temp;
 ...
 *      |__SPI_PCS_INACTIVE -片选无效,PCS信号变为高电平
 */
void LPLD_SPI_Master_Write(SPI_Type *spix, uint8_t data, uint8_t pcsx, uint8_t pcs_state)
{
    spix->PUSHR = (((uint32_t)(((uint32_t)(pcs_state)) << SPI_PUSHR_CONT_SHIFT)) & SPI_PUSHR_CONT_MASK)
                  | SPI_PUSHR_CTAS(0)
                  | SPI_PUSHR_PCS(pcsx)
