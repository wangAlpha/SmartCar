...
typedef struct io_sdcard_struct {
    uint32_t   TIMEOUT;        //响应溢出时间
    uint32_t   NUM_BLOCKS;     //设备块数
    boolean  HC;             //是否支持高容量快寻址
    boolean  VERSION2;       //是否支持规范2.0
    uint32_t   ADDRESS;        //卡地址
    uint32_t   CARD;           //实际卡类型
...
SDHCRES LPLD_SDHC_IOC(uint32_t, void *);
SDHCRES LPLD_SDHC_ReadBlocks(uint8_t *, uint32_t, uint32_t);
SDHCRES LPLD_SDHC_WriteBlocks(uint8_t *, uint32_t, uint32_t);
