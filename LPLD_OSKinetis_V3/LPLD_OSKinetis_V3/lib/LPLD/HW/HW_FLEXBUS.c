...
const uint32_t *FB_PORT[32] = {(uint32_t *) &PORTD->PCR[6], (uint32_t *) &PORTD->PCR[5], (uint32_t *) &PORTD->PCR[4], (uint32_t *) &PORTD->PCR[3],
                               (uint32_t *) &PORTD->PCR[2], (uint32_t *) &PORTC->PCR[10], (uint32_t *) &PORTC->PCR[9], (uint32_t *) &PORTC->PCR[8],
                               (uint32_t *) &PORTC->PCR[7], (uint32_t *) &PORTC->PCR[6], (uint32_t *) &PORTC->PCR[5], (uint32_t *) &PORTC->PCR[4],
                               (uint32_t *) &PORTC->PCR[2], (uint32_t *) &PORTC->PCR[1], (uint32_t *) &PORTC->PCR[0], (uint32_t *) &PORTB->PCR[18],
                               (uint32_t *) &PORTB->PCR[17], (uint32_t *) &PORTB->PCR[16], (uint32_t *) &PORTB->PCR[11], (uint32_t *) &PORTB->PCR[10],
                               (uint32_t *) &PORTB->PCR[9], (uint32_t *) &PORTB->PCR[8], (uint32_t *) &PORTB->PCR[7], (uint32_t *) &PORTB->PCR[6],
                               (uint32_t *) &PORTC->PCR[15], (uint32_t *) &PORTC->PCR[14], (uint32_t *) &PORTC->PCR[13], (uint32_t *) &PORTC->PCR[12],
                               (uint32_t *) &PORTB->PCR[23], (uint32_t *) &PORTB->PCR[22], (uint32_t *) &PORTB->PCR[21], (uint32_t *) &PORTB->PCR[20]
                              };
 ...
 *    1--配置成功
 */
uint8_t LPLD_FlexBus_Init(FB_InitTypeDef fb_init_struct)
{
    FBx Fbx = fb_init_struct.FB_Fbx;
    uint32_t base_addr = fb_init_struct.FB_ChipSelAddress;
    uint32_t addr_space = fb_init_struct.FB_AddressSpace;
    uint8_t size = fb_init_struct.FB_PortSize;
    boolean justied = fb_init_struct.FB_IsRightJustied;
    boolean aa = fb_init_struct.FB_AutoAckEnable;
    uint8_t ws = fb_init_struct.FB_WateStates;
    uint8_t rah = fb_init_struct.FB_ReadAddrHold;
    uint8_t wah = fb_init_struct.FB_WriteAddrHold;
    uint32_t *pcr;
    uint8_t n = 0;
    //参数检查
