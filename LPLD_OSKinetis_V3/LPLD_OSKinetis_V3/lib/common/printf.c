...
int32_t
printk(PRINTK_INFO *, const int8_t *, va_list);
...
/********************************************************************/
static void printk_putc(int32_t c, int32_t *count, PRINTK_INFO *info)
{
    switch (info->dest) {
    case DEST_CONSOLE:
        info->func((int8_t)c);
...
/********************************************************************/
static int32_t printk_mknumstr(int8_t *numstr, void *nump, int32_t neg, int32_t radix)
{
    int32_t a, b, c;
    uint32_t ua, ub, uc;
    int32_t nlen;
...
/********************************************************************/
static void printk_pad_zero(int32_t curlen, int32_t field_width, int32_t *count, PRINTK_INFO *info)
{
    int32_t i;
...
/********************************************************************/
int32_t printk(PRINTK_INFO *info, const int8_t *fmt, va_list ap)
{
    /* va_list ap; */
    int8_t *p;
    int32_t c;
    ...
    int32_t field_width;
#if 0
    int32_t precision_used;
    int32_t precision_width;
    int32_t length_modifier;
#endif
    int32_t ival;
    ...
    for (p = (int8_t *)fmt; (c = *p) != 0; p++) {
        ...
        cont_xd:
            ...
            break;
        case 'c':
            cval = (int8_t)va_arg(ap, uint32_t);
            printk_putc(cval, &count, info);
            break;
        case 's':
            sval = (int8_t *)va_arg(ap, int8_t *);
            if (sval) {
                vlen = strlen(sval);
                if (!IS_FLAG_MINUS(flags_used)) {
                    printk_pad_space(vlen, field_width, &count, info);
...
/********************************************************************/
int32_t sprintf(int8_t *s, const int8_t *fmt, ...)
{
    va_list ap;
    int32_t rvalue = 0;
    PRINTK_INFO info;
