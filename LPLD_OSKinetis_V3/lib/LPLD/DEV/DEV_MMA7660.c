 ...
 *    |__ 具体写入的字节型数据
 */
void LPLD_MMA7660_WriteReg(uint8_t RegisterAddress, uint8_t Data)
{
    //发送从机地址
 ...
 *      加速传感器的测量值
 */
uint8_t LPLD_MMA7660_ReadReg(uint8_t RegisterAddress)
{
    uint8_t result;
    //发送从机地址
    LPLD_I2C_StartTrans(MMA7660_I2CX, MMA7660_DEV_ADDR, I2C_MWSR);
 ...
 * 函数返回值：加速度值（int8_t）
 */
int8_t LPLD_MMA7660_GetResult(uint8_t Regs_Addr)
{
    char ret;
