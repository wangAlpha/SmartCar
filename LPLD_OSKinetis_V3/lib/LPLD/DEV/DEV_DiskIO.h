...
/* 磁盘控制函数声明 */
DSTATUS LPLD_Disk_Initialize(uint8_t);
DSTATUS LPLD_Disk_Status(uint8_t);
DRESULT LPLD_Disk_Read(uint8_t, uint8_t *, uint32_t, uint8_t);
#if _READONLY == 0
DRESULT LPLD_Disk_Write(uint8_t, const uint8_t *, uint32_t, uint8_t);
#endif
DRESULT LPLD_Disk_IOC(uint8_t, uint8_t, void *);
