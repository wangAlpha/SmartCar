...
void LPLD_SYSTICK_Init(SYSTICK_InitType systick_init_structure)
{
    ...
    uint32_t ldval = systick_init_structure.SYSTICK_PeriodUs * SYSTICK_UINT_US(g_core_clock)
...
*   无
*/
void LPLD_SYSTICK_DelayUs(uint32_t period_us)
{
    uint32_t i;
    SYSTICK->CSR = 0 & (~SYSTICK_CSR_ENABLE_MASK)      //禁止SYSTICK
                   & (~SYSTICK_CSR_TICKINT_MASK)       //禁止产生内核SYSTICK异常
...
*   无
*/
void LPLD_SYSTICK_DelayMs(uint32_t period_ms)
{
    uint32_t i;
    SYSTICK->CSR = 0 & (~SYSTICK_CSR_ENABLE_MASK)    //禁止SYSTICK
                   & (~SYSTICK_CSR_TICKINT_MASK)       //禁止产生内核SYSTICK异常
