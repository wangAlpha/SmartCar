...
#endif
static uint8_t LPLD_ADC_Cal(ADC_Type *);
 ...
 *    1--配置成功
 */
uint8_t LPLD_ADC_Init(ADC_InitTypeDef adc_init_structure)
{
    uint8_t i;
 ...
 *    1--配置成功
 */
uint8_t LPLD_ADC_Deinit(ADC_InitTypeDef adc_init_structure)
{
    ADC_Type *adcx = adc_init_structure.ADC_Adcx;
    uint8_t hwtrg = adc_init_structure.ADC_HwTrgCfg;
 ...
 *      |__AD30          --单端(VREFL)
 * 输出:
 *    AD通道转换值(右对齐)，若为差分转换结果，则为二进制补码格式(需强制转换为int16_t)
 *
 */
uint16_t LPLD_ADC_Get(ADC_Type *adcx, AdcChnEnum_Type chn)
{
 ...
 *
 */
void LPLD_ADC_EnableConversion(ADC_Type *adcx, AdcChnEnum_Type chn, uint8_t ab, boolean irq)
{
    //参数检查
 ...
 *
 */
uint16_t LPLD_ADC_GetResult(ADC_Type *adcx, uint8_t ab)
{
    //参数检查
    ASSERT(ab <= 1);  //判断AB控制寄存器
    return adcx->R[ab];
 ...
 *
 */
uint8_t LPLD_ADC_GetSC1nCOCO(ADC_Type *adcx)
{
 ...
 *
 */
uint8_t LPLD_ADC_EnableIrq(ADC_InitTypeDef adc_init_structure)
{
    uint8_t i;
    ADC_Type *adcx = adc_init_structure.ADC_Adcx;
    if (adcx == ADC0) {
 ...
 *
 */
uint8_t LPLD_ADC_DisableIrq(ADC_InitTypeDef adc_init_structure)
{
    uint8_t i;
    ADC_Type *adcx = adc_init_structure.ADC_Adcx;
    if (adcx == ADC0) {
 ...
 *
 */
uint8_t LPLD_ADC_Chn_Enable(ADC_Type *adcx, AdcChnEnum_Type chn)
{
    //判断复用引脚是a或b
 ...
 *    1--配置成功
 */
static uint8_t LPLD_ADC_Cal(ADC_Type *adcx)
{
    uint16_t cal_var;
    //为自动校准配置参数
    //为提高校准精度，需配置为：
