...
void LPLD_UART_Init(UART_InitTypeDef uart_init_structure)
{
    ...
    uint32_t baud = uart_init_structure.UART_BaudRate;
    ...
    uartx->BDL = (uint8_t)(sbr & UART_BDL_SBR_MASK);
    //配置波特率的微调分数
    ...
    if ((uartx->C2 & UART_C2_RE_MASK) == 0
        ...
        //读取 FIFO buffer 深度
        len = (uint8_t)(uartx->PFIFO & UART_PFIFO_RXFIFOSIZE_MASK) >> UART_PFIFO_RXFIFOSIZE_SHIFT;
        len = len > 0 ? 1 << (len + 1) : 1;
    ...
    if ((uartx->C2 & UART_C2_TE_MASK) == 0
        ...
        //读取 FIFO buffer 深度
        len = (uint8_t)(uartx->PFIFO & UART_PFIFO_TXFIFOSIZE_MASK) >> UART_PFIFO_TXFIFOSIZE_SHIFT;
        len = len > 0 ? 1 << (len + 1) : 1;
 ...
 *
 */
int8_t LPLD_UART_GetChar(UART_Type *uartx)
{
    //等待数据接收
 ...
 *  1       已经接收到字符
 */
int32_t LPLD_UART_GetChar_Present(UART_Type *uartx)
{
 ...
 *
 */
void LPLD_UART_PutChar(UART_Type *uartx, uint8_t ch)
{
    //等待FIFO准备就绪
 ...
 *
 */
void LPLD_UART_PutCharArr(UART_Type *uartx, uint8_t *ch, int32_t len)
{
    while (len--) {
