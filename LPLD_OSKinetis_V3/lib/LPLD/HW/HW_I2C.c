 ...
 *    1--配置成功
 */
uint8_t LPLD_I2C_Init(I2C_InitTypeDef i2c_init_structure)
{
    ...
    i2cx->C1 = I2C_C1_IICEN_MASK | I2C_C1_TXAK_MASK;      //使能I2Cx
    return 1;
}
 ...
 *
 */
uint8_t LPLD_I2C_Deinit(I2C_InitTypeDef i2c_init_structure)
{
    I2C_Type *i2cx = i2c_init_structure.I2C_I2Cx;
 ...
 *    无
 */
void LPLD_I2C_WaitAck(I2C_Type *i2cx, uint8_t is_wait)
{
    uint32_t time_out = 0;
    if (is_wait == I2C_ACK_ON) {
        while (!(i2cx->S & I2C_S_IICIF_MASK)) {
            if (++time_out > 500) { //如果等待超时，强行退出
                break;
            }
        }
 ...
 *
 */
void LPLD_I2C_WriteByte(I2C_Type *i2cx, uint8_t data8)
{
    i2cx->D = data8;
}
...
uint8_t LPLD_I2C_ReadByte(I2C_Type *i2cx)
{
    uint8_t temp;
    temp = i2cx->D;
    return temp;
 ...
 *    无
 */
void LPLD_I2C_SetMasterWR(I2C_Type *i2cx, uint8_t mode)
{
    if (mode == I2C_MRSW) {
        i2cx->C1 &= (~I2C_C1_TX_MASK);
    }
 ...
 *    无
 */
void LPLD_I2C_StartTrans(I2C_Type *i2cx, uint8_t addr, uint8_t mode)
{
    //I2C产生start信号
    LPLD_I2C_Start(i2cx);
    //将从机地址和主机读写位合成一个字节写入
