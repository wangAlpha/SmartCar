...
typedef struct {
    ...
    */
    uint8_t PDB_TriggerInputSourceSel;
...
uint8_t LPLD_PDB_Init(PDB_InitTypeDef);
//PDBx模块反初始化函数，禁用PDBx所有功能
void LPLD_PDB_Deinit(void);
//PDBx触发ADCx模块配置函数
uint8_t LPLD_PDB_AdcTriggerCfg(ADC_Type *, uint32_t, uint16_t);
//PDBx触发DACx模块配置函数
uint8_t LPLD_PDB_DacTriggerCfg(DAC_Type *, uint32_t, uint16_t);
