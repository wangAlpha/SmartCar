...
static uint8_t LPLD_FTM_PWM_Init(FTM_InitTypeDef);
static uint8_t LPLD_FTM_IC_Init(FTM_InitTypeDef);
static uint8_t LPLD_FTM_QD_Init(FTM_InitTypeDef);
static uint8_t LPLD_FTM_DEC_Init(FTM_InitTypeDef);
static uint8_t LPLD_FTM_PinInit(FTM_Type *, FtmChnEnum_Type, PortPinsEnum_Type);
static uint8_t LPLD_FTM_PinDeinit(FTM_Type *, FtmChnEnum_Type);
 ...
 *    1--配置成功
 */
uint8_t LPLD_FTM_Init(FTM_InitTypeDef ftm_init_structure)
{
    uint8_t result, i;
    //参数检查
 ...
 *    1--配置成功
 */
uint8_t LPLD_FTM_Deinit(FTM_InitTypeDef ftm_init_structure)
{
    if (ftm_init_structure.FTM_Ftmx == FTM0) {
        SIM->SCGC6 &= ~SIM_SCGC6_FTM0_MASK;
    }
 ...
 *    1--配置成功
 */
uint8_t LPLD_FTM_PWM_Enable(FTM_Type *ftmx, FtmChnEnum_Type chn, uint32_t duty, PortPinsEnum_Type pin, uint8_t align)
{
    uint32_t cv;
    vuint32 mod;
    //参数检查
 ...
 *    1--配置成功
 */
uint8_t LPLD_FTM_PWM_ChangeDuty(FTM_Type *ftmx, FtmChnEnum_Type chn, uint32_t duty)
{
    uint32_t cv;
    vuint32 mod;
    //参数检查
 ...
 *    1--配置成功
 */
uint8_t LPLD_FTM_DisableChn(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
 ...
 *    1--配置成功
 */
uint8_t LPLD_FTM_IC_Enable(FTM_Type *ftmx, FtmChnEnum_Type chn, PortPinsEnum_Type pin, uint8_t capture_edge)
{
    if (!LPLD_FTM_PinInit(ftmx, chn, pin)) {
        return 0;
    }
 ...
 *
 */
__INLINE uint16_t LPLD_FTM_GetChVal(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
 ...
 *
 */
__INLINE uint8_t LPLD_FTM_GetClkDiv(FTM_Type *ftmx)
{
 ...
 *
 */
__INLINE uint16_t LPLD_FTM_GetCounter(FTM_Type *ftmx)
{
 ...
 *
 */
uint8_t LPLD_FTM_EnableIrq(FTM_InitTypeDef ftm_init_structure)
{
    uint8_t i;
 ...
 *
 */
uint8_t LPLD_FTM_DisableIrq(FTM_InitTypeDef ftm_init_structure)
{
    uint8_t i;
    FTM_Type *ftmx = ftm_init_structure.FTM_Ftmx;
    if (ftmx == FTM0) {
 ...
 *    1--配置成功
 */
uint8_t LPLD_FTM_QD_Enable(FTM_Type *ftmx, PortPinsEnum_Type pha, PortPinsEnum_Type phb)
{
 ...
 *    1--配置成功
 */
uint8_t LPLD_FTM_QD_Disable(FTM_Type *ftmx)
{
 ...
 * FTM模块PWM模式初始化，内部调用
 */
static uint8_t LPLD_FTM_PWM_Init(FTM_InitTypeDef ftm_init_structure)
{
    uint32_t bus_clk_hz;
    uint32_t mod, mod2;
    uint8_t ps;
 ...
 * FTM模块输入捕获模式初始化，内部调用
 */
static uint8_t LPLD_FTM_IC_Init(FTM_InitTypeDef ftm_init_structure)
{
    uint8_t i;
 ...
 */
static uint8_t LPLD_FTM_PinInit(FTM_Type *ftmx, FtmChnEnum_Type chn, PortPinsEnum_Type pin)
{
    //根据ftmx使能相应pin的ftm功能
    if (ftmx == FTM0) {
        switch (chn) {
 ...
 */
static uint8_t LPLD_FTM_PinDeinit(FTM_Type *ftmx, FtmChnEnum_Type chn)
{
 ...
 * FTM模块正交解码模式初始化，内部调用
 */
static uint8_t LPLD_FTM_QD_Init(FTM_InitTypeDef ftm_init_structure)
{
...
static uint8_t LPLD_FTM_DEC_Init(FTM_InitTypeDef ftm_init_structure)
{
    return 0;
}
