 ...
 *    无
 */
static void LPLD_SDHC_InitGPIO(uint32_t init)
{
 ...
 *    无
 */
static void LPLD_SDHC_SetBaudrate(uint32_t clock, uint32_t baud)
{
 ...
 *    状态标志
 */
static uint32_t LPLD_SDHC_WaitStatus(uint32_t mask)
{
    uint32_t result;
    do {
 ...
 */
static SDHCRES LPLD_SDHC_Init(uint32_t coreClk, uint32_t baud)
{
 ...
 *    SDHCRES--磁盘功能返回值
 */
SDHCRES LPLD_SDHC_IOC(uint32_t cmd, void *param_ptr)
{
    ...
    switch (cmd) {
        ...
        case IO_IOCTL_ESDHC_GET_BAUDRATE:
            if (NULL == param32_ptr) {
                ...
                *param32_ptr = (uint32_t)((g_core_clock * 1000 / val));
            }
            break;
 ...
 *    SDHCRES--磁盘功能返回值
 */
static SDHCSTATUS LPLD_SDHC_Read(uint8_t *data_ptr, int32_t n)
{
    uint32_t buffer;
    int32_t remains;
    remains = n;
    if (((uint32_t)data_ptr & 0x03) == 0) {
        ...
        while (remains >= 4) {
            ...
            *((uint32_t *)data_ptr) = SDHC->DATPORT;
            data_ptr += 4;
            remains -= 4;
        }
    }
 ...
 *    SDHCRES--磁盘功能返回值
 */
static SDHCSTATUS LPLD_SDHC_Write(uint8_t *data_ptr, int32_t n)
{
    uint8_t *udata_ptr;
    uint32_t buffer;
    int32_t remains;
    //复制数据指针
    udata_ptr = (uint8_t *)data_ptr;
    remains = n;
    if (((uint32_t)udata_ptr & 0x03) == 0) {
        ...
        while (remains >= 4) {
            ...
            SDHC->DATPORT = *((uint32_t *)udata_ptr);
            udata_ptr += 4;
            remains -= 4;
        }
    }
 ...
 *    SDHCRES--磁盘功能返回值
 */
SDHCRES LPLD_SDHC_ReadBlocks(uint8_t *buff, uint32_t sector, uint32_t count)
{
    ESDHC_COMMAND_STRUCT command;
    int cnt;
    int32_t result;
 ...
 *    SDHCRES--磁盘功能返回值
 */
SDHCRES LPLD_SDHC_WriteBlocks(uint8_t *buff, uint32_t sector, uint32_t count)
{
    ESDHC_COMMAND_STRUCT command;
    uint8_t               tmp[4];
    int32_t             cnt;
