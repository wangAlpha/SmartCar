...
uint8_t LPLD_FTM_Init(FTM_InitTypeDef);
//FTM模块通用反初始化
uint8_t LPLD_FTM_Deinit(FTM_InitTypeDef);
//FTM模块PWM模式输出使能，配置输出通道、占空比、指定对应的引脚、对齐方式
uint8_t LPLD_FTM_PWM_Enable(FTM_Type *, FtmChnEnum_Type, uint32_t, PortPinsEnum_Type, uint8_t);
//变更FTM模块PWM模式指定通道的输出占空比
uint8_t LPLD_FTM_PWM_ChangeDuty(FTM_Type *, FtmChnEnum_Type, uint32_t);
//FTM模块输入捕获模式输出使能，配置输入通道、指定对应的引脚、捕获边缘
uint8_t LPLD_FTM_IC_Enable(FTM_Type *, FtmChnEnum_Type, PortPinsEnum_Type, uint8_t);
//禁用FTM模块指定通道的输出、输入
uint8_t LPLD_FTM_DisableChn(FTM_Type *, FtmChnEnum_Type);
//FTM模块正交解码模式输入使能，配置输入通道PHA和PHB
uint8_t LPLD_FTM_QD_Enable(FTM_Type *, PortPinsEnum_Type, PortPinsEnum_Type);
//FTM模块正交解码模式输入禁用
uint8_t LPLD_FTM_QD_Disable(FTM_Type *);
...
//获取FTMx通道n捕获的FTMx计数值
uint16_t LPLD_FTM_GetChVal(FTM_Type *, FtmChnEnum_Type);
//获取FTMx时钟分频系数
uint8_t LPLD_FTM_GetClkDiv(FTM_Type *);
//获取FTMx计数器值
uint16_t LPLD_FTM_GetCounter(FTM_Type *);
//清空FTMx计数器
void LPLD_FTM_ClearCounter(FTM_Type *);

//使能FTMx中断
uint8_t LPLD_FTM_EnableIrq(FTM_InitTypeDef);
//禁用FTMx中断
uint8_t LPLD_FTM_DisableIrq(FTM_InitTypeDef);
