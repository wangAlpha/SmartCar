...
//DAC初始化函数，使能DAC各项功能
uint8_t LPLD_DAC_Init(DAC_InitTypeDef);
//DAC反初始化函数，禁用DAC模块
uint8_t LPLD_DAC_Deinit(DAC_InitTypeDef);
//使能DACx中断
uint8_t LPLD_DAC_EnableIrq(DAC_InitTypeDef);
//禁用DACx中断
uint8_t LPLD_DAC_DisableIrq(DAC_InitTypeDef);
//设置DAC缓存区单个数据n
void LPLD_DAC_SetBufferDataN(DAC_Type *, uint16_t, uint8_t);
//设置DAC缓存区所有数据
void LPLD_DAC_SetBufferData(DAC_Type *, uint16_t *, uint8_t);
//软件触发DAC缓冲区工作
