 ...
 *    1--配置成功
 */
uint8_t LPLD_RTC_Init(RTC_InitTypeDef rtc_init_structure)
{
    int delay;
    uint32_t seconds = rtc_init_structure.RTC_Seconds;
    uint32_t alarm_time = rtc_init_structure.RTC_AlarmTime;
 ...
 *    0，RTC计数器以复位
 */
uint8_t LPLD_RTC_IsRunning(void)
{
    uint8_t request;
    SIM->SCGC6 |= SIM_SCGC6_RTC_MASK;
