...
//PDB时钟预分频系数乘数
const uint8_t pdb_sc_mults[4] = {1, 10, 20, 40};
 ...
 *    1--配置成功
 */
uint8_t LPLD_PDB_Init(PDB_InitTypeDef pdb_init_struct)
{
    uint32_t us = pdb_init_struct.PDB_CounterPeriodUs
                  + pdb_init_struct.PDB_CounterPeriodMs * 1000
                  + pdb_init_struct.PDB_CounterPeriodS * 1000000;
    uint32_t delayus = pdb_init_struct.PDB_DelayUs
                       + pdb_init_struct.PDB_DelayMs * 1000
                       + pdb_init_struct.PDB_DelayS * 1000000;
    uint8_t loadmode = pdb_init_struct.PDB_LoadModeSel;
    uint8_t trriger = pdb_init_struct.PDB_TriggerInputSourceSel;
    uint32_t bus_clk = g_bus_clock / 1000000;
    uint8_t prescaler, mult;
    uint32_t mod;
    float32 temp;
    //参数检查
    ...
    if (delayus <= us) {
        ...
        PDB0->IDLY = (uint32_t)(delayus * temp);
    }
    //加载值
 ...
 *    1--配置成功
 */
uint8_t LPLD_PDB_AdcTriggerCfg(ADC_Type *adcx, uint32_t cfg, uint16_t delay)
{
    uint8_t n, m;
    uint8_t prescaler, mult;
    uint32_t bus_clk = g_bus_clock / 1000000;
 ...
 *    1--配置成功
 */
uint8_t LPLD_PDB_DacTriggerCfg(DAC_Type *dacx, uint32_t cfg, uint16_t int_delay)
{
    uint8_t n;
    uint8_t prescaler, mult;
    uint32_t bus_clk = g_bus_clock / 1000000;
