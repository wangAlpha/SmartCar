...
const uint8_t month_list[12][3] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
 ...
 *
 */
uint32_t LPLD_DateTimeToTimeStamp(TimeStamp_FormatTypeDef *realtime_init_struct)
{
    uint16_t i;
    uint32_t current_second = 0;
    uint16_t current_year = realtime_init_struct->year;
    uint8_t current_month = realtime_init_struct->month;
    uint8_t current_day = realtime_init_struct->day;
    uint8_t current_hour = realtime_init_struct->hour;
    uint8_t current_minute = realtime_init_struct->minute;
    //判断时间起点终端是否越界
    ...
    for (i = 0; i < (current_month - 1); i++) {
        current_second += (uint32_t)month_table[i] * DAY_CONVERTTO_SECONEDS;
        if (IS_LEAPYEAR(current_year) && i == FEBRUARY) {
            current_second += DAY_CONVERTTO_SECONEDS;
        }
    }
    //将天，小时，分转化为秒
    current_second += (uint32_t)(current_day - 1) * DAY_CONVERTTO_SECONEDS;
    current_second += (uint32_t)current_hour * HOUR_CONVERTTO_SECONEDS;
    current_second += (uint32_t)current_minute * MINUTE_CONVERTTO_SECONEDS;
 ...
 *
 */
uint8_t LPLD_TimeStampToDateTime(uint32_t rtc_second, TimeStamp_FormatTypeDef *realtime_init_struct)
{
    static uint32_t day_counter = 0;
    uint32_t days = 0;
    uint32_t i = 0;
 ...
 *
 */
uint8_t LPLD_GetWeek(TimeStamp_FormatTypeDef *realtime_init_struct)
{
    uint16_t i;
    uint8_t yh, yl, day, month;
